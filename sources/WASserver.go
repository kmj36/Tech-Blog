...
type adminauthtemp struct {
	Adminid       string `json:"adminid"`
	Adminpassword string `json:"adminpassword"`
}

type checkadmintemp struct {
	AuthSHA256 string `json:"auth_cookie"`
	Apitime    string `json:"time"`
}

func apis(api *gin.Engine, idinfo requriedinfo) { // 에러 시 패닉 사용

	api.POST("/api/adminauth", func(c *gin.Context) { // 관리자 인증
		nowtime := time.Now().UTC().Format(time.RFC1123)
		var tryauthinfo adminauthtemp

		binderr := c.BindJSON(&tryauthinfo)
		if binderr != nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"code":   http.StatusBadRequest,
				"status": "Invalid or Missing ID, Password",
			})
			log.Panicln(binderr)
			return
		}

		tryauthtemp := fmt.Sprintf("%x", sha256.Sum256([]byte(base64.StdEncoding.EncodeToString([]byte(tryauthinfo.Adminid))+base64.StdEncoding.EncodeToString([]byte(tryauthinfo.Adminpassword)))))

		if tryauthtemp == idinfo.AdminSHA256 {
			cookie := fmt.Sprintf("%x", sha256.Sum256([]byte(tryauthtemp+base64.StdEncoding.EncodeToString([]byte(nowtime))+"YES")))
			...
			c.JSON(http.StatusUnauthorized, gin.H{
				...
				"auth":   "No",
				"status": "ID or Password does not match.",
			})
		}
	})

	api.POST("/api/checkadmin", func(c *gin.Context) { // 관리자 체크
		var clientauth checkadmintemp

		binderr := c.BindJSON(&clientauth)
		if binderr != nil {
			c.JSON(http.StatusUnauthorized, gin.H{
				"code":   http.StatusUnauthorized,
		...
		if clientauth.AuthSHA256 == fmt.Sprintf("%x", sha256.Sum256([]byte(idinfo.AdminSHA256+base64.StdEncoding.EncodeToString([]byte(clientauth.Apitime))+"YES"))) {
			c.JSON(http.StatusOK, gin.H{
				"code": http.StatusOK,
				"auth": "Yes",
			})
	...
	api.POST("/api/uploadpost", func(c *gin.Context) { // 게시물 업로드
		...
		db, openerr := sql.Open("mysql", idinfo.Dbid+":"+idinfo.Dbpass+"@tcp(127.0.0.1:"+idinfo.Dbport+")/"+idinfo.Dbname) // db 오픈
		if openerr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"code":   http.StatusInternalServerError,
				"status": "Server Error.",
	...
	api.GET("/api/posts", func(c *gin.Context) { // 게시물 조회
		...
		db, openerr := sql.Open("mysql", idinfo.Dbid+":"+idinfo.Dbpass+"@tcp(127.0.0.1:"+idinfo.Dbport+")/"+idinfo.Dbname)
		if openerr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"code":   http.StatusInternalServerError,
				"status": "Server Error.",
	...
	api.POST("/api/editpost", func(c *gin.Context) { // 게시물 수정
		var temp postDetail
	...
	api.POST("/api/deletepost", func(c *gin.Context) { // 게시물 삭제
	...
	api.POST("/api/comments", func(c *gin.Context) { // 댓글 업로드
	...
	api.GET("/api/comments", func(c *gin.Context) { // 댓글 가져오기
	...
	api.POST("/api/deletecomment", func(c *gin.Context) { // 댓글 삭제
	...
	api.POST("/api/imageup", func(c *gin.Context) { // 이미지 처리
...
func handles(pagehandle *gin.Engine, idinfo requriedinfo) {
	pagehandle.GET("/", func(c *gin.Context) { // 리다이렉트 유도
		c.Data(http.StatusOK, "text/html", []byte(`
			<html lang = "ko">
				...
				<body>
					<p>Redirect : <a href=`+idinfo.Serverptc+`://`+idinfo.Serverip+`:`+idinfo.Clientport+`>`+idinfo.Serverptc+`://`+idinfo.Serverip+`</a></p>
				</body>
			</html>
		`))
	})
...
func Middleware(idinfo requriedinfo) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.Header("Access-Control-Allow-Origin", idinfo.Serverptc+"://"+idinfo.Serverip /*+":"+idinfo.Clientport 개발용*/)
		...
		c.Header("Expires", time.Now().Add(time.Minute*60).UTC().Format(time.RFC1123))
...
func main() { // 에러 시 페이탈 사용
	...
	for i := 0; i < 2; i++ {
		idscanner.Scan()
		reflect.ValueOf(&admintemp).Elem().Field(i).SetString(idscanner.Text()) // 관리자 id, password 파싱
	}

	idinfo.AdminSHA256 = fmt.Sprintf("%x", sha256.Sum256([]byte(base64.StdEncoding.EncodeToString([]byte(admintemp.Adminid))+base64.StdEncoding.EncodeToString([]byte(admintemp.Adminpassword))))) // 클라이언트 관리자 보안
	admintemp.Adminid = ""
	admintemp.Adminpassword = ""
	...
	/* 필수 정보 검증 */
	db, connerr := sql.Open("mysql", idinfo.Dbid+":"+idinfo.Dbpass+"@tcp(127.0.0.1:"+idinfo.Dbport+")/"+idinfo.Dbname)
	if connerr != nil {
		log.Fatalln(connerr)
	}
	db.Close()
	...
	ownerr := data.Chown(useruid, usergid) // 로그 파일의 현재소유자 변경
	if ownerr != nil {
		log.Fatalln(ownerr)
	}
